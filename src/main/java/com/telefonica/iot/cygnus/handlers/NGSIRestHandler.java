...
public class NGSIRestHandler extends CygnusHandler implements HTTPSourceHandler {
    ...
    public void configure(Context context) {
        ...
        String jwtSecret = context.getString(NGSIConstants.PARAM_JWT_SECRET, null);
        ...
        if (jwtSecret != null) {
            ...
            try {
                ...
                invalidConfiguration = true;
                LOGGER.error("Bad configuration ('" + NGSIConstants.PARAM_JWT_SECRET
                        + "' is an invalid key");
            } catch (NoSuchAlgorithmException e) {
                invalidConfiguration = true;
                LOGGER.error("Bad configuration ('" + NGSIConstants.PARAM_JWT_SECRET
                        + "' No such algorithm available HmacSHA256");
            }
        }
    ...
    public List<Event> getEvents(javax.servlet.http.HttpServletRequest request) throws Exception {
        ...
        while (headerNames.hasMoreElements()) {
            ...
            } else if (headerName.equals(NGSIConstants.NGSI_V2_ATTRS_FORMAT)) {
                ngsiv2AttrsFormat = headerValue;
            } else if (headerName.equals(NGSIConstants.X_AUTHORIZATION)) {
                if (!headerValue.contains("Bearer")) {
                    LOGGER.warn("Bad HTTP notification (X-Authorization must be in the format Bearer [jwt_token])");
                    throw new HTTPBadRequestException("X-Authorization must be in the format Bearer [jwt_token]");
                } else {
        ...
        // check if received authorizationToken is null and the authorization system is on

        JwtClaims claims = null;
        if (authorizationRequired) {

            if (authorizationToken == null) {
                LOGGER.warn("Missing X-Authorization. Required 'Bearer [jwt_token]'");
                throw new HTTPBadRequestException("Missing X-Authorization. Required 'Bearer [jwt_token]'");
            } // if

            JwtConsumer jwtConsumer = new JwtConsumerBuilder()
                    .setRequireSubject() // the JWT must have a subject claim
                    .setExpectedIssuer("Viur") // whom the JWT needs to have been issued by
                    .setExpectedAudience("Cygnus") // to whom the JWT is intended for
                    .setExpectedSubject("Cygnus Authorization")
                    .setVerificationKey(this.secretKeySpec) // verify the signature with the public key
                    .build(); // create the JwtConsumer instance

            claims = NGSIUtils.getJWTClaims(authorizationToken, jwtConsumer);

            if (claims == null) {
                LOGGER.warn("X-Authorization token is invalid");
                throw new HTTPBadRequestException("X-Authorization token is invalid");
            }
        ...
        LOGGER.debug("Adding flume event header (name=" + NGSIConstants.FLUME_HEADER_TRANSACTION_ID
                + ", value=" + transId + ")");
        eventHeaders.put(NGSIConstants.NGSI_V2_ATTRS_FORMAT, ngsiv2AttrsFormat);
        LOGGER.debug("Adding flume event header (name=" + NGSIConstants.NGSI_V2_ATTRS_FORMAT
                + ", value=" + ngsiv2AttrsFormat + ")");

        if (authorizationRequired && claims != null) {

            String schema = claims.getStringClaimValue("schema");

            eventHeaders.put(CommonConstants.HEADER_FIWARE_SERVICE, schema);
            LOGGER.debug("Adding flume event header (name=" + CommonConstants.HEADER_FIWARE_SERVICE
                    + ", value=" + schema + ")");
        }
